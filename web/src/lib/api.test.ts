...
describe("fetchWithTimeout", () => {
  ...
  it("returns response when fetch resolves before timeout", async () => {
    ...
    const res = await fetchWithTimeout("https://test.com/api/data", {
      timeout: 5000,
    });
  ...
  it("aborts when parent signal is aborted", async () => {
    ...
    await expect(
      fetchWithTimeout("https://test.com/api", {
...
describe("fetchWithRetry", () => {
  ...
  it("throws immediately on non-retryable error", async () => {
    vi.spyOn(globalThis, "fetch").mockRejectedValueOnce(
      Object.assign(new Error("Syntax error"), { name: "SyntaxError" }),
    );
    await expect(
      fetchWithRetry("https://test.com/api", { retries: 1 }),
...
describe("api object", () => {
  ...
  it("api.live.categories() returns parsed JSON on success", async () => {
    const data = {
      categories: [{ category_id: "1", category_name: "News", parent_id: 0 }],
    };
