...
export function useSwipeToGoBack() {
  ...
  const getBackUrl = useCallback((type: "live" | "movie" | "series") => {
    let backUrl = "";
    try {
      backUrl = sessionStorage.getItem("stv_back_url") || "";
  ...
  const handleTouchStart = useCallback(
    (e: React.TouchEvent, centerTouched: React.MutableRefObject<boolean>) => {
  ...
  const handleTouchEnd = useCallback(
    (e: React.TouchEvent, type: "live" | "movie" | "series") => {
      if (!swipeStart.current) return;
      const dx = (e.changedTouches[0]?.clientX || 0) - swipeStart.current.x;
      const dy = Math.abs(
        (e.changedTouches[0]?.clientY || 0) - swipeStart.current.y,
      );
      if (dx > 80 && dx > dy * 1.5) {
        goBack(type);
      }
