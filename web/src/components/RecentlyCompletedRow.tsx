...
export default function RecentlyCompletedRow({
  navigate,
}: RecentlyCompletedRowProps) {
  ...
  const completed = items.filter(
    (i) =>
      i.durationSeconds > 0 && i.progressSeconds / i.durationSeconds >= 0.9,
  );
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        className="flex gap-3 overflow-x-auto scrollbar-none pb-1 pr-4 md:pr-0"
        ...
        {enriched.map((item) => (
          ...
          >
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                removeSeriesProgress(item.seriesId);
                setItems((prev) =>
                  prev.filter(
                    (i) =>
                      !(
