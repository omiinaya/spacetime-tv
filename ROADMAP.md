...
| **Frontend quality** | B+ | 86% | ↑ 85% | Movies.tsx reduced from 689→552 lines. 68 test files (+2), 1223 tests (+14). New MovieContinueWatchingRow and MovieRecentlyCompletedRow components with dedicated tests. AudioSelector flaky test fixed. |
||| **Backend architecture** | B | 80% | ↑ 73% | Ruff lint 0 errors (was 137). Missing `asyncio` import fixed (production bug). Missing `ProviderConfig` import fixed. Dead code removed. `contextlib.suppress` replaces bare try/except in stream modules. state.py coverage 100%. |
