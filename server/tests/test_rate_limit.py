...
def test_search_path_rate_limit_applied(small_limits, client):
    """/api/search blocks after RATE_SEARCH_LIMIT (3) requests from same IP."""
...
def test_image_proxy_rate_limit(small_limits, client):
    """/api/image-proxy shares RATE_SEARCH_LIMIT with search path."""
    # First 3 requests to image-proxy should succeed
    for _ in range(3):
        resp = client.get("/api/image-proxy?url=http://example.com/img.jpg")
...
def test_image_proxy_and_search_share_limit(small_limits, client):
    ...
    resp = client.get("/api/search?q=c")
    assert resp.status_code == 429
...
def test_rate_limit_resets_after_window_expiry(small_window, client):
    ...
    client.get("/api/search?q=test")
    client.get("/api/search?q=test")
    assert client.get("/api/search?q=test").status_code == 429
