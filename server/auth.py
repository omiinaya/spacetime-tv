...
def add_profile_favorite(profile_id: str, item: dict) -> bool:
    ...
    profiles[profile_id]["favorites"] = [
        f for f in profiles[profile_id]["favorites"] if (f.get("watchKey") or f.get("id", "")) != watch_key
    ]
...
def generate_profile_token(profile_id: str, device_id: str = "") -> str:
    ...
    sig = hmac.new(_get_token_secret().encode(), payload.encode(), hashlib.sha256).hexdigest()[:16]
...
def verify_profile_token(token: str) -> dict | None:
    ...
    try:
        ...
        if time.time() > expiry:
            return None
        payload = f"{profile_id}:{device_id}:{expiry}"
        expected = hmac.new(_get_token_secret().encode(), payload.encode(), hashlib.sha256).hexdigest()[:16]
