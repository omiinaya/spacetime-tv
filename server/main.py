...
async def live_info(
    ...
    """Batch stream info: name + icon for given IDs. Fast — reads from cached live_all."""
    requested = set()
...
async def _enrich_tmdb_item(item_type: str, tmdb_id: str) -> dict | None:
    ...
    if item_type == "movie" and os.getenv("TMDB_API_KEY"):
        from routes.tmdb import tmdb_fetch
...
# Uses the browserless tmdb-enrich CLI tool. No API key needed.
...
async def guide_enrich(
    ...
    try:
        proc = await asyncio.create_subprocess_exec(
            "/home/user/.local/share/hermes-cli-tools-venv/bin/tmdb-enrich", "--json", "enrich", q,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
        )
        stdout, stderr = await asyncio.wait_for(proc.communicate(), timeout=20)
