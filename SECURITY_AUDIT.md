...
| **Auth Coverage** | 93/100 | 🟢 Low | All `/api/*` routes require X-Admin-Key or X-Device-Token via middleware. LAN bypass gated by `ALLOW_LAN_BYPASS` (default true; set false = hardened). Exempt: health/error, cloud-backup registration, profiles list/verify/session (by design — pre-auth profile picker). Profile detail **and** every profile write handler self-enforce `_require_profile_access` (X-Profile-Token or admin key) since middleware skips `/api/v1/profiles/*`. PIN auth has a per-profile brute-force lockout (5 fails/30s). |
